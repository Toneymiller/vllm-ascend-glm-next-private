...
def make_thread(**overrides: object) -> MooncakePullRecvingThread:
    ...
    thread.cache_roles = ["kv"]
    thread.transfer_unit_tokens = [16]
    ...
    if "cache_roles" not in overrides:
        thread.cache_roles = ["kv"] * len(thread.layer_names)
    if "transfer_unit_tokens" not in overrides:
        thread.transfer_unit_tokens = [16] * len(thread.layer_names)
    return thread
