...
class MooncakeBaseConnectorScheduler:
    ...
    def _needs_prefill_token_truncation(self) -> bool:
        ...
        uses_compressed_cache = any(
            getattr(spec, "compress_ratio", 1) > 1 for group_specs in self.group_unique_specs for spec in group_specs
        )
        has_state_group = any(
            self._is_state_spec(spec) for group_specs in self.group_unique_specs for spec in group_specs
        )
        return uses_compressed_cache or has_state_group

    @staticmethod
    def _is_state_spec(spec: Any) -> bool:
        return isinstance(spec, MambaSpec) or getattr(spec, "cache_role", None) in (
            "state",
            "indexer_state",
        )
    ...
    def _get_transfer_block_ids(self, block_ids: BlockIds, prompt_len: int) -> BlockIds:
        ...
        for blocks, block_size, group_specs in zip(
            ...
            if any(self._is_state_spec(spec) for spec in group_specs):
                if any(isinstance(spec, MambaSpec) for spec in group_specs) and self.num_speculative_tokens > 0:
